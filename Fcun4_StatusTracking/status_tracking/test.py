...
def app_process_creater(job_title: str, company_name: str, job_description: str, tracking_method: str) -> str:
    ...
    try:
        ...
        if app_status_file:
            ...
            app_status = pd.DataFrame(columns=["job_id", "status_update", "timestamp"])
        new_app_status = pd.concat([app_status, pd.DataFrame([[job_id, "开始流程跟踪。", time.time()]], columns=app_status.columns)])
...
def is_app_new(job_title: str | None = None, company_name: str | None = None) -> bool:
    ...
    :param job_title: 求职岗位的职位名称。
    :param company_name: 求职岗位的公司名称。
    :return: 返回是否已记录该职位，False表示没有记录，True表示有记录。
    """
...
class AppProcessUpdaterSchema(BaseModel):
    job_title: str = Field(description="求职岗位的职位名称。")
    company_name: str = Field(description="求职岗位的公司名称。")
    status: str = Field(description="求职岗位的申请状态。")

@tool(args_schema=AppProcessUpdaterSchema)
def app_process_updater(job_title: str, company_name: str, status: str) -> str:
    """
    ...
    该函数可以在求职流程信息记录文件中新增该职位当前的申请状态。
    :param job_title: 求职岗位的职位名称。
    :param company_name: 求职岗位的公司名称。
    :param status: 求职岗位的申请状态。
    :return: 返回记录结果。
    """
    file_root = Path("./tables")
    app_status_file_name = f"{username}_application_status.csv"
    ...
    if app_status_file and summary_file:
        ...
        new_status = pd.DataFrame([[job_id, status, time.time()]], columns=app_status.columns)
...
3. **网申流程状态更新：**
   ...
   - 你需要根据用户提供的信息抽取出求职岗位的职位名称，公司名称，申请状态，输入到`app_process_updater`中即可。
...
# create agent
memory = MemorySaver()
agent = create_react_agent(model=model, tools=tools, prompt=prompt, checkpointer=memory, debug=True)
...
# Name of company/institution: 上海阶跃星辰智能科技有限公司 Position: 大语言模型预训练算法研究员 Job Description: 1. 参与大语言模型的算法研究和开发工作，包括但不限于模型架构设计、优化和 改进； 2. 负责大语言模型的预训练和调优，包括数据预处理、模型训练和评估等； 3. 参与大语言模型的评测和分析工作，包括建立 in-house 的测评集、编写评测代 码、分析评测结果等； 4. 参与大语言模型的应用开发工作，包括编写应用代码、优化应用性能等； 5. 参与撰写相关技术报告和论文。 Requirements: 1. 熟练掌握 Python 编程语言，熟悉机器学习和深度学习的基本原理和算法； 2. 熟悉自然语言处理的基本原理和算法，有相关项目经验者优先； 3. 熟悉大语言模型的原理和应用，了解当前大语言模型的最新进展； 4. 有良好的团队合作精神和沟通能力，能够与团队成员密切合作，共同推进项目 进展。
# Name of company/institution: 国家集成电路创新中心 NICIC Position: 软件-图像算法工程师 (Intern) Job Description: Work with chip design engineers to design image classification, recognition, and tracking algorithm models that are compatible with efficient edge hardware. For example, we tailored models such as YOLO and OS Track, split the unit operations of these operators, and evaluated and improved the performance of these operators (speed and hardware resource usage). Needed: 1. Hardworking 2. Self-driven 3. Good expressive skills. Training and Guidance: A mentor will be assigned and basic training and reference documents will be provided in the first two weeks. Key Domain(s)/Skills Category: Multimedia Computing, Artificial Intelligence, Programming, Algorithms and Machine Learning Other Domain(s)/Skills: Python Programming, Office Word/EXCEL/PPT, Thesis Writing
# Name of company/institution: 上海阶跃星辰智能科技有限公司 Position: 语音（多模态）算法研究员 Job Description: 1. 参与语音合成模型的调优和性能分析，协助发现和解决实际应用中的技术问题； 2. 进行大规模语音数据的预处理、标注和数据增强工作，支持模型的训练和评估； 3. 快速学习并应用最新的研究成果，开展实验，发表高质量学术论文。 Requirements: 1. 在读本科或研究生，计算机科学、电子工程、信息科学、信号处理或相关专业； 2. 熟悉机器学习和深度学习的基本原理，有使用深度学习框架如 TensorFlow, PyTorch 的经验； 3. 熟练掌握 Python，有良好的编程习惯和代码风格； 4. 积极主动，具有较强的学习能力和解决问题的能力； 5. 良好的团队合作和沟通能力，能够在导师的指导下进行研究和开发工作。
# Name of company/institution: ONERWAY Position: 数据分析实习生 Job Description: 1. 协助日常数据分析事项：对业务数据进⾏探索和分析，发现潜在规律、趋势和 问题点，并定期向团队汇报分析结果； 2. 协助建模：在导师指导下，参与算法模型的搭建、验证和初步应用工作（如客 户分群、预测模型等），从而提升分析效率； 3. 协助风控运营：负责日常交易风险监控和异常处理，优化告警规则，配置风控 调整策略； 4. 参与数据平台建设：使⽤SUPERSET 工具制作全英文的可视化看板； 5. 其他数据支持事项：包括使用 SQL 等工具从公司数据库中提取数据，业务数据 答疑等。 Key Domain(s)/Skills Category: /
# Name of company/institution: 广发基金 Position: 量化研究实习生 (深度学习、机器学习、强化学习方向） Job Description: 利用 AI 技术对金融市场数据进行深入分析，构建预测市场价格、波动性、情绪等的模 型（如 Transformer、GCN、多任务等）；能够诊断模型问题（如过拟合、欠拟合、 梯度问题、收敛性）并运用合适技术解决；设计模型评估方案（IC、IR、收益、稳定 性、解释性等），并推动模型诊断与迭代。 Requirements: 1. 计算机科学、数学、物理、统计学相关专业硕士或以上学历（在读），拥有扎 实的数学基础和计算机编程能力； 2. 熟悉深度学习算法和模型，具备创新研究能力； 3. 良好的逻辑思维、沟通协调和自我学习能力，主动负责，严谨细致，勤奋踏实。
# Name of company/institution: ONERWAY Position: AI 应用开发实习生 Job Description: 1. 开发内部通用场景的 AI 应用，包括但不限于：基于 RAG 的知识库检索及后端 数据的离线链路，为合规/运营/审核使用的 AI 提效工具等； 2. 负责自研的基于 React 框架下的多 AGENT 交互框架； 3. 整合优化内部已有的 AI 应用，基于公有云搭建公司的 AI 应用生态，建立完整的 架构支持长期的 AI 应用迭代。